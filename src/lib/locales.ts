...
export const getNativeLabel = (code: CountryCode): string => {
  return SEO_LOCALE_META[code]?.label || code.toUpperCase();
};

export const getHrefLang = (code: CountryCode): string => {
  return SEO_LOCALE_META[code]?.hreflang || `${code}-${code.toUpperCase()}`;
};
...
export const DEFAULT_LOCALE: SiteLocale = "us";
