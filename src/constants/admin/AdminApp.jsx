...
export default function AdminApp() {
  ...
  const userRows = users.map((user) => ({
    title: `${user.name} ${user.verified ? "✓" : ""}`.trim(),
  ...
  const marketplaceRows = agents
    .filter((agent) => agent.visibility === "marketplace")
    .sort((a, b) => b.installs - a.installs)
    .map((agent) => ({
      title: agent.name,
      meta: `${agent.installs.toLocaleString()} installs • ★ ${agent.rating} • ⭐ ${agent.priceStars}`,
      badge: agent.type,
    }));
    ...
    <div className="min-h-screen bg-[linear-gradient(135deg,#d9d6ff_0%,#dde8ff_50%,#d8f0ff_100%)] text-slate-900">
      <div className="mx-auto grid max-w-7xl grid-cols-[280px_1fr] gap-4 p-4">
        ...
        <div className="space-y-4">
          ...
          {tab === "marketplace" && (
            <DataList title="Marketplace" rows={marketplaceRows} />
          )}
