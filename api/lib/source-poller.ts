...
const POLLER_RUN_EVERY_MS = 30 * 1000; // запускать не чаще чем раз в 30 сек
const POLLER_LOCK_TTL_SECONDS = 25; // lock чуть меньше интервала
const SOURCES_PER_RUN = 40; // сколько источников берём за один запуск
const POSTS_PER_SOURCE_SCAN = 12; // сколько последних postId читаем со страницы канала
const MAX_NEW_POSTS_PER_SOURCE = 3; // сколько реально импортируем за один проход с одного канала
const SOURCE_CONCURRENCY = 6; // сколько каналов обрабатываем параллельно
